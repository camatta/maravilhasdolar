...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const payload = {
      ...
      // novos campos para o template do e-mail
      anexo: originalName,
  ...
  if (isSubmitted) {
    return (
      <div className={styles.containerFaleConosco} style={{ background: '#F0FFF4', border: '1px solid #A7F3D0', borderRadius: 12, padding: 16 }}>
        <strong style={{ color: '#065F46' }}>Recebido com sucesso!</strong>
        <div style={{ color: '#065F46' }}>Em breve entraremos em contato.</div>
      </div>
    )
  }
  ...
  return (
    <div className={styles.containerFaleConosco} ref={firstErrorRef}>
      ...
      <form className={styles.formContato} onSubmit={handleSubmit} noValidate style={{ display: 'grid', gap: 14 }}>
        {formError && (
          <div role="alert" style={{ background: '#FFF3F3', border: '1px solid #F5C2C7', color: '#842029', borderRadius: 8, padding: 12 }}>
            {formError}
          </div>
        )}
        ...
        <div>
          ...
          />
          {errors.nome && <small id="err-nome" style={{ color: '#c0392b', fontSize: 12 }}>{errors.nome}</small>}
        </div>
        ...
        <div>
          ...
          />
          {errors.email && <small id="err-email" style={{ color: '#c0392b', fontSize: 12 }}>{errors.email}</small>}
        </div>
        ...
        <div>
          ...
          />
          {errors.telefone && <small id="err-telefone" style={{ color: '#c0392b', fontSize: 12 }}>{errors.telefone}</small>}
        </div>
        ...
        <div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
            <small id="mensagem-count" aria-live="polite" style={{ color: '#6b7280' }}>
        ...
        <div>
          <input
            ...
            // opcional: restrinja tipos se quiser, por exemplo: accept=".pdf,image/*"
          />
          <div id="anexo-help" style={{ marginTop: 4, color: '#6b7280', fontSize: 12 }}>
            Tamanho máximo: {MAX_FILE_MB} MB.
            {formData.anexo && (
              <span> Arquivo selecionado: {formData.anexo.name} ({formatMB(formData.anexo.size)} MB)</span>
            )}
          </div>
          {errors.anexo && <small id="err-anexo" style={{ color: '#c0392b', fontSize: 12 }}>{errors.anexo}</small>}
        </div>

        <button
          type="submit"
          disabled={isLoading}
          style={{ width: '100%', height: 45, backgroundColor: isLoading ? '#ccc' : '#dd5600', border: 'none', borderRadius: 2, color: '#fff', fontSize: 16, fontWeight: 700, cursor: isLoading ? 'not-allowed' : 'pointer' }}
        >
